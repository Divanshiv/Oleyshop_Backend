...
;
        ...
        $orders = $orders->notPos()->orderBy('id','desc')
            ->paginate($perPage)
            ->appends($queryParam);
        ...
        return view('branch-views.order.list', compact('orders', 'status','search', 'countData', 'startDate', 'endDate', 'perPage'));
    }
    ...
    public function paymentStatus(Request $request): RedirectResponse
    {
        ...
        if ($order->payment_status == $request->payment_status) {
            Toastr::warning(translate('Payment status is already set'));
            return back();
        }

        if (in_array($order->order_status, ['returned', 'delivered', 'failed', 'canceled'])) {
            Toastr::warning(translate('you_can_not_change_the_payment_status_of '. $order->order_status .' order'));
            return back();
        }
    ...
    public function updateTimeSlot(Request $request)
    {
        ...
        if ($order->order_status === 'delivered') {
            return response()->json([
                'message' => translate('Delivered orders cannot be modified'),
            ], 403);
        }
    ...
    public function updateDeliveryDate(Request $request)
    {
        $request->validate([
            ...
            'deliveryDate.after_or_equal' => translate('Delivery date cannot be a past date'),
        ]);
...
;
    ...
    public function verifyOfflinePayment(Request $request, $order_id, $status)
    {
        $offlineData = OfflinePayment::where(['order_id' => $order_id])->first();

        if (!isset($offlineData)){
            Toastr::warning(translate('offline data not found'));
            return back();
        }
        ...
        if ($status == 1){
            if (in_array($order->order_status, ['returned', 'delivered', 'failed', 'canceled'])) {
                Toastr::warning(translate('you_can_not_verify_the_payment_of '. $order->order_status .' order'));
                return back();
            }
            ...
            $request->validate(
                [
                    'denied_note' => ['required', 'string', 'max:255', 'regex:/\S+/',
                    ],
                ],
                [
                    'denied_note.regex' => translate('Deny note cannot be empty or only spaces'),
                ]
            );

            $offlineData->status = $status;
            $offlineData->denied_note = $request->denied_note;
            $offlineData->save();
            ...
            if ($customerFcmToken != null) {
                try {
                    $data = [
                        'title' => translate('Order'),
                        'description' => translate('Offline payment is denied'),
                        'order_id' => $order->id,
                        'image' => '',
                        'type' => 'order',
                    ];
    ...
    public function offlinePaymentList(Request $request, $status): Factory|View|Application
    {
        ...
        $queryParam = ['per_page' => $perPage, 'search' => $search];
        ...
        return view('branch-views.order.offline-payment.list', compact('orders', 'search', 'perPage'));
    }
    ...
    public function switchPaymentMethod($id)
    {
        $order = Order::find($id);
        ...
        if ($order->offline_payment) {
            $order->offline_payment->delete();
        }

        if ($order->partial_payment->isNotEmpty()) {
            // Delete rows where `paid_with` is not 'wallet_payment'
        ...
        if ($customerFcmToken != null) {
            $data = [
                'title' => translate('Order'),
                'description' => translate('Payment method change to Cash On Delivery by Branch Admin'),
                'image' => '',
                'order_id' => $order->id,
                'type' => 'order'
            ];
        ...
        Toastr::success(translate('Payment method switch successfully.'));
        return back();
    }
}
