...
class OrderController extends Controller
{
    ...
    public function list(Request $request, $status): View|Factory|Application
    {
        ...
        $orders = $query->notPos()->orderBy('id', 'desc')->paginate($perPage)->appends($queryParam);
        ...
        return view('admin-views.order.list', compact('orders', 'status', 'search', 'branches', 'branchId', 'startDate', 'endDate', 'countData','perPage'));
    }
    ...
    public function switchPaymentMethod($id)
    {
        ...
        if ($customerFcmToken != null) {
            $data = [
                'title' => translate('Order'),
                'description' => translate('Payment method change to Cash On Delivery by Admin'),
                'image' => '',
                'order_id' => $order->id,
                'type' => 'order'
            ];
