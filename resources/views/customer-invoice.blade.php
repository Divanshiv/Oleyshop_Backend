...
<body class="footer-offset">
    ...
    <div class="container">
        <div class="content container-fluid initial-38" id="printableArea">
            ...
            <div class="initial-38-1">
                        ...
                        @else
                            @if($order->order_type == 'delivery')
                                @if(isset($order->delivery_address))
                                    ...
                                    <h5>
                                        {{ translate('Customer Phone') }} :<span class="font-light">{{$order->delivery_address['contact_person_number']}}</span>
                                    </h5>
                                    @if(isset($order->delivery_address['address']))
                                        <h5 class="text-break">
                ...
                @if ($order['order_type'] !== 'pos' && !empty($order['order_note']))
                    <h5>
                        ...
                        : <span class="font-light"> {{ $order['order_note'] }} </span>
                    </h5>
                @endif

                @if($order['bring_change_amount'])
                    <h5><strong>{{ translate('Bring Change Note') }}</strong>: <span class="font-light">{{translate('Please ensure the Deliveryman has '). \App\CentralLogics\Helpers::set_symbol($order['bring_change_amount']) . translate(' in change ready for the customer')}}</span></h5>
                @endif
                    ...
                    @foreach($order->details as $detail)
                        ...
                        @if($detail->product_details !=null)
                            ...
                            <tr>
                                ...
                                <td class="">
                                    {{$product['name'] ?? translate('Product not available') }} <br>
                ...
                <div class="px-3">
                    <dl class="row text-right justify-content-center">
                        ...
                        @if($order->order_change_amount()->exists())
                            <dt class="col-6">{{translate('Paid Amount')}}<span>:</span></dt>
                            ...
                            <dt class="col-6">{{$changeOrDueAmount < 0 ? translate('Due Amount') : translate('Change Amount') }}<span>:</span></dt>
