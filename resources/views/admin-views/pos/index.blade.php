            ...
            <div class="order--pos-left">
                <div class="card">
                    <div class="card-header bg-light border-0">
                        <h5 class="card-title">{{ translate('Product section') }}</h5>
                    </div>
                    <div class="card-body">
            ...
            <div class="order--pos-right">
                <div class="card">
                    <div class="card-header bg-light border-0 m-1">
                        <h5 class="card-title">{{ translate('Billing section') }}</h5>
                    </div>
                        ...
                        <div class="row g-2" id="delivery_address">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="contact_person_name"
                                    ...
                                    placeholder="{{ translate('Ex :') }} {{ translate('John') }}" required>
                            </div>
                            <div class="col-md-6">
...
@push('script_2')
    ...
    <script>
        ...
        function updateExtraDiscount() {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('admin.pos.update-extra-discount') }}',
                data: form.serialize(),
                dataType: 'json',
                ...
                error: function(xhr) {
                    if (xhr.status === 422) {
                        ...
                        if (errors.discount) {
                            $('#discount_error').text(errors.discount[0]);
                        } else {
                            $('#discount_error').text('');
                        }
                    } else {
        ...
        $(document).ready(function() {
            $(document).on('click', '#extra_discount_submit_btn', function(e) {
                e.preventDefault();
                updateExtraDiscount();
            });
            ...
            $(document).on('click', '#confirm-delete-discount', function() {
                $.ajax({
                    url: "{{ route('admin.pos.delete-extra-discount') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
