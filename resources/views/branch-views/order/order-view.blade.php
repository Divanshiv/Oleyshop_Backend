...
@section('content')
                    ...
                    <div class="card-header flex-wrap align-items-start border-0">
                        ...
                        <div class="w-100">
                            ...
                            @if($order['bring_change_amount'])
                                <h6><strong>{{ translate('Bring Change Note') }}</strong>: <span class="text-body">{{translate('Please ensure the Deliveryman has '). \App\CentralLogics\Helpers::set_symbol($order['bring_change_amount']) . translate(' in change ready for the customer')}}</span></h6>
                            @endif
                        </div>
                    </div>
                                            ...
                                            <td>
                                                <div class="media media--sm">
                                                    ...
                                                    <div class="media-body">
                                                        <h5 class="line--limit-1">{{$product['name'] ?? translate('Product not available') }}</h5>
                                                        ...
                                                        <h5 class="mt-1"><span class="text-body">{{translate('Unit Price')}}</span> : {{ Helpers::set_symbol($detail['price']) }} </h5>
                            ...
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right justify-content-end">
                                    ...
                                    @if($order->order_change_amount()->exists())
                                        ...
                                        <dd class="col-6 col-xl-5 pr-5">{{ Helpers::set_symbol($order->order_change_amount?->paid_amount) }}</dd>
                                        ...
                                        <dd class="col-6 col-xl-5 pr-5">{{ Helpers::set_symbol($changeOrDueAmount) }}</dd>
                                    @endif
                                </dl>
                            </div>
                        </div>
                                ...
                                @if($order->offline_payment)
                                                            ...
                                                            @foreach ($item as $subKey => $subItem)
                                                                @if (is_array($subItem))
                                                                    @foreach ($subItem as $deepKey => $deepVal)
                                                                        <span><strong>{{ translate($deepKey) }}:</strong> {{ translate($deepVal) }}</span><br>
                                                                    @endforeach
                                                                @else
                                                                    <span><strong>{{ translate($subKey) }}:</strong> {{ translate($subItem) }}</span><br>
                                                                @endif
                                                            @endforeach
                                                        </div>
                                                    @else
                                    ...
                                    @if($order->offline_payment?->status == 0)
                                        <div class="d-flex flex-wrap justify-content-center gap-3 mt-4">
                                            <button class="btn badge-danger bg-opacity-danger-10 text-danger flex-grow-1" data-toggle="modal"
                                                    data-target="#deniedModal-{{$order->id}}">{{ translate('deny') }}</button>
                                            <a class="btn badge-info flex-grow-1 text-info bg-opacity-info-10" href="{{route('branch.orders.verify-offline-payment', ['order_id' => $order->id, 'status' => 1])}}">{{ translate('approve') }}</a>
                                        </div>
                                    @elseif($order->offline_payment?->status == 2)
                                        @if($order['order_status'] != 'canceled' && $order['order_status'] != 'failed' && $order['order_status'] != 'return' && $order['order_status'] != 'delivered')
                                            <div class="d-flex flex-column gap-2 mt-4">
                                                <a class="btn btn--primary w-100" href="{{ route('branch.orders.switch-payment',['id'=>$order['id']]) }}">{{ translate('Switch to Cash On Delivery') }}</a>
                                                <a class="btn btn-danger w-100 bg-opacity-danger-10 text-danger" href="{{ route('branch.orders.status',['id'=>$order['id'],'order_status'=>'canceled']) }}">{{ translate('Cancel Order') }}</a>
                                            </div>
                                        @endif
                                    @endif
                                @else
                                    <img src="{{ asset('public/assets/admin/img/offline-payment.png') }}" alt="Payment Icon" class="mb-3">
                                    <p class="">{{ translate('Customer did not submit any payment information yet') }}</p>
                                    @if($order['order_status'] != 'canceled' && $order['order_status'] != 'failed' && $order['order_status'] != 'return' && $order['order_status'] != 'delivered')
                                        <div class="d-flex flex-column gap-2 mt-4">
                                            <a class="btn btn--primary w-100" href="{{ route('branch.orders.switch-payment',['id'=>$order['id']]) }}">{{ translate('Switch to Cash On Delivery') }}</a>
                                            <a class="btn btn-danger w-100 bg-opacity-danger-10 text-danger" href="{{ route('branch.orders.status',['id'=>$order['id'],'order_status'=>'canceled']) }}">{{ translate('Cancel Order') }}</a>
                        ...
                        @else
                            ...
                            @if(isset($order->customer) )
                                <div class="media align-items-center deco-none customer--information-single">
                                    ...
                                    <div class="media-body">
                                                    <span class="fs--14px text--title font-semibold text-hover-primary d-block">
                                                        <a href="#">{{$order->customer['f_name'].' '.$order->customer['l_name']}}</a>
                                                    </span>
    ...
    <div class="modal fade" id="deniedModal-{{$order->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body pt-5 p-md-5">
                    ...
                    <form method="GET" action="{{route('branch.orders.verify-offline-payment',['order_id' => $order->id, 'status' => 2])}}">
                        <div>
