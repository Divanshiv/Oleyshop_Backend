                            ...
                            <div class="row g-1">
                                ...
                                @foreach ($products as $product)
                                    <div class="order--item-box item-box">
                                        @include('branch-views.pos._single_product', [
                                            'product' => $product,
                        ...
                        <div class="px-4">
                            <div class="d-flex flex-wrap flex-row py-2 add--customer-btn">
                                ...
                                <button class="btn btn--primary rounded font-regular" data-toggle="modal"
                                    data-target="#add-customer" type="button">{{ translate('Add New Customer') }}</button>
                            </div>
                            <div class="w-100 py-2">
                                <div class="form-group">
                                    <label
                                        class="input-label font-weight-semibold fs-16 text-dark">{{ translate('Select Order Type') }}</label>
                                    <div>
                                        <div class="form-control d-flex flex-column-3">
                                            <label class="custom-radio d-flex gap-2 align-items-center m-0">
                                                {{-- type="radio" => order_type | after submit add to cart this input add in formData --}}
                            ...
                            <div class="w-100 py-2">
                                <div class="form-group d-none" id="home_delivery_section">
                                    <div class="d-flex justify-content-between">
                                        <label for=""
                                            class="font-weight-semibold fs-16 text-dark">{{ translate('Delivery Information') }}
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="text-center">
                            <input id="printButton" type="button" class="btn btn-primary non-printable"
                ...
                <div class="modal-body">
                    ...
                    <form id='delivery_address_store'>
                        ...
                        <div class="col-md-12 mt-2">
                            <div class="btn--container justify-content-end">
                                <button id="updateDeliveryAddressButton" class="btn btn-sm btn--primary w-100"
    ...
    <script>
        ...
        function cartQuantityInitialize() {
                ...
                if (!isNaN(currentVal)) {
                    if (type == 'minus') {
                        ...
                        if (currentVal + 1 > qty_initial_max_val) {
                            $('.product-stock-count').html( qty_initial_max_val);
                            showWarningPopup();
                            if(qty_initial_max_val !=0){
            ...
            $('.input-number').change(function() {
                ...
                if (valueCurrent >= input_qty_max_val) {
                    $('.stock-validation-message').html('There isn’t enough quantity on stock. only ' +
                        input_initial_max_val + ' items are available.');
                    showWarningPopup();
        ...
        function getVariantPrice(initial = false) {
            ...
            if (initial) {
                $.ajax({
                    type: 'GET',
                    url: '{{ route('branch.pos.quick-view-modal-footer') }}',
                    data: formData,
                    success: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#add-to-cart-form #chosen_price_div #chosen_price').html(data.price.toFixed(2));
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.data == 1) {
                            ...
                            toastr.success('{{ translate('Item has been updated in your cart') }}!', {
                                CloseButton: true,
                                ProgressBar: true
                            });
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: "{{ translate('Cart') }}",
                                text: "{{ translate('Sorry, Product is out of stock') }}",
                                confirmButtonText: '{{ translate('Ok') }}',
                            });
        ...
        function updateUI(productId) {
            return $.post('{{ route('branch.pos.cart_items') }}', {
                _token: '{{ csrf_token() }}'
            }).done(function(data) {
                $('#cart').empty().html(data);
        ...
        function updateQuantity(e) {
            ...
            if (!isNaN(valueCurrent)) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: '{{ translate('Cart') }}',
                    text: '{{ translate('Kindly provide valid quantity') }}',
                    confirmButtonText: '{{ translate('Ok') }}',
                });
        ...
        function updateExtraDiscount() {
            let form = $('#update_extra_discount_form');

            $.ajax({
                type: "POST",
                url: '{{ route('branch.pos.update-extra-discount') }}',
                data: form.serialize(),
                dataType: 'json',
                success: function(response) {
                    if (response.success) {
                        $('#update-extra-discount').modal('hide');
                ...
                error: function(xhr) {
                    if (xhr.status === 422) {
                        ...
                        if (errors.discount) {
                            $('#extra_discount_error').text(errors.discount[0]);
                        } else {
                            $('#extra_discount_error').text('');
                        }
                    } else {
        ...
        $(document).ready(function() {
            $(document).on('click', '#extra_discount_update_btn', function(e) {
                e.preventDefault();
                updateExtraDiscount();
            });

            $(document).on('keypress', '#extra_discount_input', function(e) {
                if (e.which === 13) {
                    e.preventDefault();
                    updateExtraDiscount();
                }
            ...
            $(document).on('click', '#confirm-delete-discount', function() {
                $.ajax({
                    url: "{{ route('branch.pos.delete-extra-discount') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(response) {
                        ...
                        $('#delete-extra-discount').modal('hide');
                        $('body').removeClass('modal-open');
                        $('.modal-backdrop').remove();
                        $('body').css('padding-right', '');
                        updateCart();
    ...
    @if ($googleMapStatus == 1 && $deliveryType == 'distance')
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                script.src =
                    "https://maps.googleapis.com/maps/api/js?key={{ \App\Model\BusinessSetting::where('key', 'map_api_client_key')->first()?->value }}&libraries=places&v=weekly";
